            ...
            while(command != '0' && command != 'q'){
                ...
                switch(command) {
                    case '1':{
                        ...
                        std::map<int, std::shared_ptr<Item>> results = d1.findComics(desiredName, maxSum, desiredCondition);
                        std::cout << std::endl;
                        for (auto i: results) {
                            std::cout << "Store: " << dynamic_cast<Comic*>(i.second.get())->getStoreBelongName() << " with the ID: ";
                            std::cout << i.first;
                            std::cout << ' ';
                            std::cout << *i.second;
                        }
...
MainInterface* MainInterface::instance = nullptr;
